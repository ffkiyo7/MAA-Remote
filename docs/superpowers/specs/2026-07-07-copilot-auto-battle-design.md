...
> 状态：**设计草案（2026-07-07 对齐；2026-07-08 跑了首批 Spike）**——核心可行性已坐实：**P0(`maa run`+Copilot 组链)/S1-API/S5 ✅ 通过**；**S2 已定案：copilot_list 不做"主页→地图",需自建导航 override**；S3/S4 未跑。详见 **§十一 Spike 执行记录**。下一步：写 `copilot_catalog.py`（唯一完全 de-risk 的切片）。
...
**核心可行性（P0/S1-API/S5）✅ 已成立**；S2 决定全自动还是需要导航开发（待活动关重测）；S3/S4 决定练度数据的精度上限。
