...
> 状态：**设计定稿**（架构 + 第 2 段模块接口已完成，详见 `SPEC.md`）
> 最后更新：2026-07-02
...
| LLM 供应商 | **DeepSeek**（成本敏感，目标 < ¥10/月；实测混合方案 < ¥1/月，忽略不计。省钱关键：缓存固定的系统提示） |
| 运行设备 | **安卓模拟器，需自动拉起**（平时不开）。工作流负责：拉起模拟器 → 等 adb 就绪 → 跑 MAA → 结束后可选关闭 |
| 日常任务范围 | 开局唤醒+收邮件 · 刷理智(fight) · 公招+信用商店 · 基建换班+领日常奖励；**再叠加**自然语言临时指定关卡/操作 |
...
| 飞书身份 | **每机不同**。运行时读 `auth status` 取当机身份，不写死。（本机 2026-07-02：bot 身份，appId `cli_a9429c3b63f89cc1`，登录用户 黄文轩；user token 过期，仅 bot 可用） |
...
关键约束：
- **一次只跑一个任务**：执行中再来消息 → 回"正在跑中，稍等"，绝不并发（防两个 maa 抢模拟器）。
- **每步都有兜底**：模拟器起不来 / adb 超时 / maa 非零退出 / DeepSeek 超时 → 都回明确失败消息，不静默。
...
3. 测试策略（含 `evals/router_cases.jsonl` 意图识别回归）。
4. 落地时需向你采集的信息：
   - 用哪个模拟器（MuMu/雷电/BlueStacks…）、安装路径、adb 端口/地址。
   - 快速路径刷理智的**固定关卡号**（如果不用"当前关"）。
   - DeepSeek API key。
   - 飞书事件订阅是否已开启（`im.message.receive_v1`）、机器人 IM 权限范围。
5. ✅ 实现计划已生成 → `docs/superpowers/plans/2026-07-02-maa-remote.md`（10 个 TDD 任务，含端到端落地校验清单）。下一步：执行计划（subagent-driven 或 inline）。
...
2. 在项目里启动 Claude Code，说："读 `docs/CONTEXT.md`，我们接着上次的 MAA_remote 设计往下讲第 2 段。"
3. 注意：项目记忆存在 `~/.claude/projects/D--Code-MAA-remote/memory/`（**不随项目目录走**）。本文件第四节已把最关键的"不写死身份"约束复制进来，所以换机也不丢。
